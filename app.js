...
function updateFabVisibility() {
  ...
  const visible = Boolean(session.role);
...
document.addEventListener("click", (event) => {
  const fabButton = event.target.closest?.("#fab-action");
  if (fabButton) {
    event.preventDefault();
    event.stopPropagation();
    if (session.role) toggleFabMenu();
    return;
  }

  const fabTarget = event.target.closest?.("[data-fab-target]");
  if (fabTarget) {
    event.preventDefault();
    event.stopPropagation();
    const target = fabTarget.dataset.fabTarget;
    closeFabMenu();
    navigateTo("control");
  ...
  const fabView = event.target.closest?.("#fab-menu [data-view-jump]");
  if (fabView) {
    event.preventDefault();
    event.stopPropagation();
    closeFabMenu();
    navigateTo(fabView.dataset.viewJump);
    return;
  }
